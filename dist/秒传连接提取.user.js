...
(() => {
    ...
    (() => {
        ...
        var htmlDocument = '<p class="mzf_text">秒传无效/md5获取失败/防和谐等 可参考<a href="' + doc.shareDoc + '" ' + linkStyle + '><span class="text" style="width: auto;">分享教程</span></a></p>';
        var htmlDonate = '<p id="mzf_donate" class="mzf_text">若喜欢该脚本, 可前往 <a href="' + donatePage + '" ' + btnStyle + '>赞助页</a> 支持作者<a id="kill_donate" class="mzf_btn">不再显示</a></p>';
        var htmlFeedback = '<p id="mzf_feedback" class="mzf_text">若有任何疑问, 可前往 <a href="' + homePage + '" ' + btnStyle + '>脚本主页</a> 反馈<a id="kill_feedback" class="mzf_btn">不再显示</a></p>';
        ...
        var updateInfo = __webpack_require__(451);
        ...
        function ajax(config, callback, failback) {
            $.ajax(__assign(__assign({}, config), {
                success: function(data, _statusTxt, jqXHR) {
                    if (jqXHR.status == 200) callback(data, jqXHR); else failback(jqXHR.status);
                },
                error: function() {
                    failback(ajaxError);
                }
        ...
        var Swalbase = function() {
            ...
            Swalbase.prototype.inputPathView = function() {
                var _this = this;
                Swal.fire(this.mergeArg(SwalConfig.inputPathView)).then((function(result) {
                    if (result.isConfirmed) {
                        _this.rapiduploadTask.savePath = result.value;
            ...
            Swalbase.prototype.processView = function(isGen) {
                var swalArg = {
                    ...
                    willOpen: isGen ? function() {} : this.saveFileWork
                };
            ...
            Swalbase.prototype.finishView = function(isGen) {
                ...
                var html = isGen ? htmlCheckMd5 + htmlDocument + (parseResult.htmlInfo && "<p><br></p>" + parseResult.htmlInfo) : htmlDocument + "<p><br></p>" + parseResult.htmlInfo;
                var htmlFooter = "";
                ...
                Swal.fire(this.mergeArg(SwalConfig.finishView, swalArg)).then((function(result) {
                    if (result.isConfirmed && (isGen || _this.rapiduploadTask.checkMode)) {
            ...
            Swalbase.prototype.genFileWork = function(isUnfinish) {
                var _this = this;
        ...
        var GeneratebdlinkTask = function() {
            ...
            GeneratebdlinkTask.prototype.start = function() {
                this.selectList.forEach((function(item) {
                    if (item.isdir) this.dirList.push(item.path); else {
                ...
                ajax({
                    ...
                    if (!data.errno) {
                        data.list.forEach((function(item) {
                            item.isdir || this.fileInfoList.push({
                                path: item.path
                            });
                        }));
            ...
            GeneratebdlinkTask.prototype.downloadFileData = function(i, dlink) {
                ...
                ajax({
                    ...
                    _this.parseDownloadData(i, data, xhr);
                }), (function(statusCode) {
                    file.errno = statusCode;
                    _this.generateBdlink(i + 1);
                }));
            };
            GeneratebdlinkTask.prototype.parseDownloadData = function(i, data, xhr) {
                var file = this.fileInfoList[i];
                if (xhr.getResponseHeader("access-control-allow-methods")) {
                    ...
                    var fileMd5 = xhr.getResponseHeader("content-md5");
                    ...
                    file.md5s = sliceMd5;
                    var interval = this.fileInfoList.length > 1 ? 2500 : 1e3;
                    setTimeout((function() {
                        this.generateBdlink(i + 1);
                    }), interval);
                }
        ...
        function loaderBaidu() {
            ...
            $(document).on("click", "#check_md5_btn", swalInstance.checkMd5);
        }
